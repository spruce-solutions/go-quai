...
func (w *worker) GeneratePendingHeader(block *types.WorkObject, fill bool, txs types.TxByPriceAndTime, fromOrderedTransactionSet bool) (*types.WorkObject, error) {
	...
	if nodeCtx == common.ZONE_CTX && w.hc.ProcessingState() {
		...
		if bytes.Equal(work.wo.PrimaryCoinbase().Bytes(), quaiCoinbase.Bytes()) {
			primaryCoinbase := w.GetPrimaryCoinbase()
			coinbaseFeeEtx := types.NewTx(&types.ExternalTx{To: &primaryCoinbase, Gas: params.TxGas, Value: halfQiFees, EtxType: types.CoinbaseType, OriginatingTxHash: common.SetBlockHashForQuai(block.Hash(), w.hc.NodeLocation()), ETXIndex: uint16(len(work.etxs)), Sender: primaryCoinbase, Data: []byte{lockupByte}})
			work.etxs = append(work.etxs, coinbaseFeeEtx)
			if work.utxoFees.Cmp(big.NewInt(0)) != 0 {
				secondaryCoinbase := w.GetSecondaryCoinbase()
				coinbaseFeeEtx := types.NewTx(&types.ExternalTx{To: &secondaryCoinbase, Gas: params.TxGas, Value: halfQuaiFees, EtxType: types.CoinbaseType, OriginatingTxHash: common.SetBlockHashForQi(block.Hash(), w.hc.NodeLocation()), ETXIndex: uint16(len(work.etxs)), Sender: w.secondaryCoinbase, Data: []byte{lockupByte}})
				work.etxs = append(work.etxs, coinbaseFeeEtx)
			}
		} else if bytes.Equal(work.wo.PrimaryCoinbase().Bytes(), qiCoinbase.Bytes()) {
			primaryCoinbase := w.GetPrimaryCoinbase()
			coinbaseFeeEtx := types.NewTx(&types.ExternalTx{To: &primaryCoinbase, Gas: params.TxGas, Value: halfQiFees, EtxType: types.CoinbaseType, OriginatingTxHash: common.SetBlockHashForQi(block.Hash(), w.hc.NodeLocation()), ETXIndex: uint16(len(work.etxs)), Sender: primaryCoinbase, Data: []byte{lockupByte}})
			work.etxs = append(work.etxs, coinbaseFeeEtx)
			if work.quaiFees.Cmp(big.NewInt(0)) != 0 {
				secondaryCoinbase := w.GetSecondaryCoinbase()
				coinbaseFeeEtx := types.NewTx(&types.ExternalTx{To: &secondaryCoinbase, Gas: params.TxGas, Value: halfQuaiFees, EtxType: types.CoinbaseType, OriginatingTxHash: common.SetBlockHashForQuai(block.Hash(), w.hc.NodeLocation()), ETXIndex: uint16(len(work.etxs)), Sender: secondaryCoinbase, Data: []byte{lockupByte}})
				work.etxs = append(work.etxs, coinbaseFeeEtx)
			}
		}
		...
		if work.wo.NumberU64(common.ZONE_CTX) > uint64(params.WorkSharesInclusionDepth) {
			...
			zoneThresholdEntropy := w.engine.IntrinsicLogEntropy(common.BytesToHash(target.Bytes()))
