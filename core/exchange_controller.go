...
func CalculateBetaFromMiningChoiceAndConversions(hc *HeaderChain, block *types.WorkObject, newTokenChoiceSet types.TokenChoiceSet) (*big.Int, error) {
	...
	if block.NumberU64(common.ZONE_CTX) < params.GoldenAgeForkNumberV3+types.C_tokenChoiceSetSize {
	...
	for _, tokenChoices := range newTokenChoiceSet {
		...
		if tokenChoices.Diff.Cmp(common.Big0) == 0 {
			panic("token choices diff is zero for the first time")
		}

		log.Global.Error("token choices set", tokenChoices)
	}
	...
	if bestDiff.Cmp(common.Big0) < 0 {
		panic("best diff cannot be negative")
	}
	if bestDiff.Cmp(common.Big0) == 0 {
		panic("best diff cannot be zero")
	}
	...
	minerDifficulty := hc.ComputeMinerDifficulty(block)
...
func CalculateTokenChoicesSet(hc *HeaderChain, block, parent *types.WorkObject, etxs types.Transactions, actualConversionAmountInHash, realizedConversionAmountInHash *big.Int, minerDifficulty *big.Int) (types.TokenChoiceSet, error) {
	...
	if minerDifficulty.Cmp(common.Big0) == 0 {
		panic("miner difficulty cannot be nil")
	}
	log.Global.Warn("Miner difficulty", minerDifficulty)
	...
	for _, tx := range etxs {
		if types.IsCoinBaseTx(tx) {
			...
			// calculated
			if tx.To().IsInQiLedgerScope() {
				tokenChoices.Qi += NormalizeConversionValueToBlock(parent, tx.Value(), true)
			} else if tx.To().IsInQuaiLedgerScope() {
				tokenChoices.Quai += NormalizeConversionValueToBlock(parent, tx.Value(), false)
			}
		}
	}
	...
	if tokenChoices.Diff.Cmp(common.Big0) == 0 {
		log.Global.Warn("Token choices diff cannot be zero")
	...
	if block.NumberU64(common.ZONE_CTX) <= params.GoldenAgeForkNumberV3+types.C_tokenChoiceSetSize {
		if hc.IsGenesisHash(block.ParentHash(common.ZONE_CTX)) || block.NumberU64(common.ZONE_CTX) == params.GoldenAgeForkNumberV3 { // parent is genesis
			...
			newTokenChoiceSet[block.NumberU64(common.ZONE_CTX)-params.GoldenAgeForkNumberV3-1] = tokenChoices
		}
	} else {
